...
def load_dataset_few(name, split_idx = 0):

    if name in ['cora', 'citeseer', 'pubmed']:
        dataset = load_planetoid_dataset_few(name)
    elif name in ['computer', 'photo']:
        dataset = load_amazon_dataset_few(name)
    elif name in ['cs', 'physics']:
        dataset = load_coauthor_dataset_few(name)
        ...
        dataset = loadwikics_few()

    return dataset


def loadwikics_few(train_ratio = 0.1, val_ratio = 0.1, test_ratio = 0.8):
    ...
    return (graph, feat, label, num_class)



def load_planetoid_dataset_few(name):
    if name == 'cora':
...
def load_amazon_dataset_few(name, train_num):
    if name == 'computer':
...
def load_coauthor_dataset_few(name, train_ratio = 0.1, val_ratio = 0.1, test_ratio = 0.8):
    if name == 'cs':
...
def get_train_data(labels, tr_num, val_num, seed):
    ...
    idx_train = torch.LongTensor(np.random.permutation(idx_train)).to(device)
    idx_val = torch.LongTensor(np.random.permutation(idx_val)).to(device)
    idx_test = torch.LongTensor(np.random.permutation(idx_test)).to(device)
