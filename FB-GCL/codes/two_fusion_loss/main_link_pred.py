...
from utils_data import load_dataset, set_random_seed, cal_adj_by_feat, load_dataset_split
...
parser.add_argument('--test_rate',  type=float, default=0.1, help='Use MLP instead of GNN')
...
def Evaluate(model, graph, g_x, feat, train_idx, val_idx, test_idx, max_epoch):
    ...
    for epoch in range(max_epoch):
        ...
        with torch.no_grad():
            ...
            if val_embs.shape[0] ==0:
                if eval_acc<test_acc:
                    best_epoch = epoch
                    eval_acc = test_acc
                    best_val_acc = 0
            else:
    ...
    print('Best val acc:{:.4f}, test acc:{:.4f}, epoch:{}'.format(best_val_acc, eval_acc, best_epoch))
    return eval_acc
...
if __name__ == '__main__':
    ...
    for seed in range(7,10):
        ...
        test_edges, test_edges_false= mask_test_edges(adj_sparse, test_frac=args.test_rate, val_frac=0.05)
        ...
        # train_ratio = 0.1
        # graph, feat, labels, num_class, train_idx, val_idx, test_idx = load_dataset_split(dataname, train_ratio, train_ratio)
        ...
        for epoch in range(epochs + 1):
            ...
            if dataname == 'physics':
                loss = model(graph_f, g_x, feat, adj_label, adj_X, adj_rec, batch_size=256)
            else:
                loss = model(graph_f, g_x, feat, adj_label, adj_X,  adj_rec)
            ...
            if epoch == 600:
                roc, ap = get_roc_score(model, graph_f, g_x, feat, test_edges, test_edges_false, adj_sparse)
                acc_list4.append(roc)
                acc_list4_ap.append(ap)
            if epoch == 700:
                roc, ap = get_roc_score(model, graph_f, g_x, feat, test_edges, test_edges_false, adj_sparse)
                acc_list1.append(roc)
                acc_list1_ap.append(ap)
            # if epoch == 300:
            #     roc, ap = get_roc_score(model, graph_f, g_x, feat, test_edges, test_edges_false, adj_sparse)
            #     acc_list2.append(roc)
            #     acc_list2_ap.append(ap)
            # if epoch == 800:
            #     roc, ap = get_roc_score(model, graph_f, g_x, feat, test_edges, test_edges_false, adj_sparse)
    ...
    print(f"# {600}_final_f1: {final_acc:.4f}±{final_acc_std:.4f}")
    final_acc, final_acc_std = np.mean(acc_list1_ap), np.std(acc_list1_ap)
    print(f"# {600}_final_f1_ap: {final_acc:.4f}±{final_acc_std:.4f}")

    final_acc, final_acc_std = np.mean(acc_list2), np.std(acc_list2)
    print(f"# {700}_final_f1: {final_acc:.4f}±{final_acc_std:.4f}")
    final_acc, final_acc_std = np.mean(acc_list2_ap), np.std(acc_list2_ap)
    print(f"# {700}_final_f1_ap: {final_acc:.4f}±{final_acc_std:.4f}")

    final_acc, final_acc_std = np.mean(acc_list3), np.std(acc_list3)
    print(f"# {800}final_f1: {final_acc:.4f}±{final_acc_std:.4f}")
    final_acc, final_acc_std = np.mean(acc_list3_ap), np.std(acc_list3_ap)
    print(f"# {800}final_f1_ap: {final_acc:.4f}±{final_acc_std:.4f}")

    final_acc, final_acc_std = np.mean(acc_list4), np.std(acc_list4)
    print(f"# {550}final_f1: {final_acc:.4f}±{final_acc_std:.4f}")
    final_acc, final_acc_std = np.mean(acc_list4_ap), np.std(acc_list4_ap)
    print(f"# {550}final_f1_ap: {final_acc:.4f}±{final_acc_std:.4f}")
